...
def set_encoder_mean_model(hidden_size,
                           feature_size=2048):
    # linear layer
    linear_w0 = weight_init((feature_size, hidden_size),
                            'enc_mean_linear_w0')
    linear_b0 = bias_init(hidden_size,
                          'enc_mean_linear_b0')
    parameter_set = [linear_w0, linear_b0]

    def encoder_mean(feature_data):
        mean = T.dot(feature_data, linear_w0) + linear_b0
        return mean

    return encoder_mean, parameter_set

def set_encoder_variance_model(hidden_size,
                               feature_size=2048):
    # linear layer
    linear_w0 = weight_init((feature_size, hidden_size),
                            'enc_mean_linear_w0')
    ...
    def encoder_variance(feature_data):
        variance = T.dot(feature_data, linear_w0) + linear_b0
        return variance

    return encoder_variance, parameter_set
...
def set_generator_model(num_hiddens):
    ...
    conv_w5   = weight_init((num_gen_filters4, num_gen_filters5) + filter_shape,
                            'gen_conv_w5')
    ...
    def sample_generator(hidden_data):
        ...
        h0_0 = deconv(relu(seed), conv_w0_0, subsample=(2, 2), border_mode=(1, 1))+conv_b0_0.dimshuffle('x', 0, 'x', 'x')
        h0_1 = deconv(relu(h0_0), conv_w0_1, subsample=(1, 1), border_mode=(1, 1))+conv_b0_1.dimshuffle('x', 0, 'x', 'x')
        h0_2 = deconv(relu(h0_1), conv_w0_2, subsample=(1, 1), border_mode=(1, 1))+conv_b0_2.dimshuffle('x', 0, 'x', 'x')
        ...
        h1_1 = deconv(relu(h1_0), conv_w1_1, subsample=(1, 1), border_mode=(1, 1))+conv_b1_1.dimshuffle('x', 0, 'x', 'x')
        h1_2 = deconv(relu(h1_1), conv_w1_2, subsample=(1, 1), border_mode=(1, 1))+conv_b1_2.dimshuffle('x', 0, 'x', 'x')
        ...
        h2_1 = deconv(relu(h2_0), conv_w2_1, subsample=(1, 1), border_mode=(1, 1))+conv_b2_1.dimshuffle('x', 0, 'x', 'x')
        h2_2 = deconv(relu(h2_1), conv_w2_2, subsample=(1, 1), border_mode=(1, 1))+conv_b2_2.dimshuffle('x', 0, 'x', 'x')
        ...
        h3_1 = deconv(relu(h3_0), conv_w3_1, subsample=(1, 1), border_mode=(1, 1))+conv_b3_1.dimshuffle('x', 0, 'x', 'x')
        ...
        h4_1 = deconv(relu(h4_0), conv_w4_1, subsample=(1, 1), border_mode=(1, 1))+conv_b4_1.dimshuffle('x', 0, 'x', 'x')

        # deconv output (64x64x64=>64x64x3)
        output = deconv(relu(h4_1), conv_w5, subsample=(1, 1), border_mode=(1, 1))+conv_b5.dimshuffle('x', 0, 'x', 'x')
        output = tanh(output)
        return [T.flatten(seed, 2),
                ...
                T.flatten(h3_0, 2), T.flatten(h3_1, 2),
                T.flatten(h4_0, 2), T.flatten(h4_1, 2),
                output]
...
def set_updater_function(feature_extractor,
    ...
    positive_random = T.matrix(name='positive_random',
                               dtype=theano.config.floatX)
    negative_hidden = T.matrix(name='negative_hidden',
                               dtype=theano.config.floatX)
    ...
    positive_mean     = encoder_mean(positive_features)
    positive_log_var  = encoder_variance(positive_features)
    positive_std      = T.sqrt(T.exp(positive_log_var))
    positive_hidden   = positive_mean + positive_std*positive_random
    # positive generate
    positive_generate = sample_generator(positive_hidden)
    ...
    # moment matching
    moment_match_cost = 0
    for i in xrange(len(positive_generate)):
        pos_feat = positive_generate[i]
        neg_feat = negative_generate[i]
    ...
    updater_updates = optimizer(encoder_parameters + generator_parameters,
                                positive_vae_cost.mean() + moment_match_cost)
